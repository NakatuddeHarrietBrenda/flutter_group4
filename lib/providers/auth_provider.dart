...
class AuthProvider with ChangeNotifier {
  ...
  // Login method integrating with the API
  Future<bool> login(String emailOrContact, String password) async {
    _isLoading = true;
    _errorMessage = null;
    notifyListeners();
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (responseData.containsKey('data')) {
          userName = responseData['data']['name'] ?? (responseData['data']['user'] != null ? responseData['data']['user']['name'] : null);
